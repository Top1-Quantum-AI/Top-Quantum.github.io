...
router.post('/apply-gate', authMiddleware, quantumLimiter, trackApiUsage(3), async (req, res) => {
  try {
    ...
    switch (gateType) {
      ...
      case 'CNOT':
        if (controlQubit === undefined || targetQubit === undefined ||
            controlQubit < 0 || controlQubit >= userState.numQubits ||
            targetQubit < 0 || targetQubit >= userState.numQubits ||
...
router.get('/states', authMiddleware, async (req, res) => {
  try {
    ...
    const states = await quantumService.getUserStates(
