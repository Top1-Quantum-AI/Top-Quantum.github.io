...
import { Atom, Brain, Network, Activity, Zap, BarChart3, Shield, AlertTriangle, Users, Workflow, Target, Layers, Sparkles, Moon, Sun, Command, Search, Palette, LayoutGrid } from 'lucide-react';
import localforage from 'localforage';
import LoginScreen from './quantum/LoginScreen';
import QuantumSidebar from './quantum/QuantumSidebar';
import type { ProcessingStates, SecureLog, UnifiedQuantumState, QuantumModule, SystemMetrics, SmartTheme, CommandAction } from './quantum/types';
import { SYSTEM_USERNAME, SYSTEM_PASSWORD } from './config/credentials';

// تُستورد أنواع البيانات من quantum/types.ts
// Types are imported from quantum/types.ts
...
const RevolutionaryQuantumSystem: React.FC = () => {
  ...
  // واجهة تسجيل الدخول — مُعاد استخدامها كمكوّن مستقل
  if (!isLoggedIn) {
    return (
      <LoginScreen
    ...
    <div className={`min-h-screen text-white overflow-hidden relative revolutionary-bg ${
      ...
      <div className="flex">
        <QuantumSidebar
          ...
          onModuleSelect={(moduleId, researchTopic) => {
            setActiveModule(moduleId);
            if (researchTopic) {
              performResearch(researchTopic, moduleId);
            }
          }}
        />
