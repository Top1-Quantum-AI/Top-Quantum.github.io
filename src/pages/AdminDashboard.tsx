...
const AdminDashboard: React.FC = () => {
  ...
  const roleBadge = (role: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${styles[role] ?? styles['user']}`}
      >
            ...
            <div className='grid grid-cols-2 lg:grid-cols-4 gap-4'>
              {[
                ...
                },
                {
                  ...
                  color: 'green',
                  sub: `${stats.users.verified} مُوثق`,
                },
                {
                  label: 'طلبات API',
                  value: stats.api.totalRequests.toLocaleString(),
                  icon: <Activity className='w-5 h-5' />,
                  color: 'purple',
                  sub: `${(stats.api.totalTokens / 1_000_000).toFixed(1)}M tokens`,
                },
                {
                  label: 'الخطط المدفوعة',
                  value: stats.plans.professional + stats.plans.enterprise,
                  icon: <Crown className='w-5 h-5' />,
                  color: 'yellow',
                  sub: `${stats.plans.enterprise} مؤسسات`,
                },
              ].map((card, i) => (
                ...
                {[
                  {
                    ...
                    pct:
                      stats.users.total > 0
                        ? Math.round((stats.plans.free / stats.users.total) * 100)
                        : 0,
                  },
                  {
                    label: 'احترافي',
                    ...
                    pct:
                      stats.users.total > 0
                        ? Math.round((stats.plans.professional / stats.users.total) * 100)
                        : 0,
                  },
                  {
                    label: 'مؤسسات',
                    ...
                    pct:
                      stats.users.total > 0
                        ? Math.round((stats.plans.enterprise / stats.users.total) * 100)
                        : 0,
                  },
                ].map((plan, i) => (
            ...
            <div className='bg-gray-900/50 border border-gray-800 rounded-xl p-6'>
              ...
              <div className='space-y-2'>
                {logs.slice(0, 8).map((log, i) => (
                  <div
                    key={i}
                    className='flex items-center justify-between py-2 px-3 rounded-lg hover:bg-gray-800/50 text-sm'
                  >
                    <div className='flex items-center gap-3'>
                      {log.success ? (
                      ...
                      <span className='text-gray-600'>
                        {log.success ? 'تسجيل دخول ناجح' : 'محاولة فاشلة'}
                      </span>
                    </div>
              ...
              {users.pagination.pages > 1 && (
                <div className='flex items-center justify-between px-4 py-3 border-t border-gray-800'>
                  <p className='text-xs text-gray-500'>
                    {users.pagination.total} مستخدم — صفحة {users.pagination.page} من{' '}
                      ...
                      >
                        <td className='py-3 px-4'>
                          {log.success ? (
                            ...
                            <span className='flex items-center gap-1 text-red-400 text-xs'>
                              <AlertTriangle className='w-3.5 h-3.5' /> فاشل
                            </span>
                          )}
                        </td>
                        <td className='py-3 px-4'>
                          <span className='text-gray-300'>{log.username}</span>
                          <span className='text-xs text-gray-600 mr-2' dir='ltr'>
                            {log.email}
                          </span>
                        </td>
                        <td className='py-3 px-4'>{roleBadge(log.role)}</td>
                        <td className='py-3 px-4 text-xs text-gray-500 font-mono' dir='ltr'>
                          {log.ip}
                        </td>
                        <td className='py-3 px-4 text-xs text-gray-500'>
            ...
            >
              ...
              <div className='flex items-center gap-3 p-3 bg-gray-800/50 rounded-xl'>
                <div className='w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center font-bold'>
                  {(
                    editingUser.profile.firstName?.[0] ??
