...
const NetworkDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const generateNodes = (): NetworkNode[] => {
      const nodeTypes: Array<'quantum' | 'classical' | 'hybrid'> = [
        'quantum',
        'classical',
        'hybrid',
      ];
      const statuses: Array<'online' | 'offline' | 'maintenance' | 'error'> = [
        'online',
        'offline',
        'maintenance',
        'error',
      ...
      return Array.from({ length: 12 }, (_, i) => ({
        ...
        status:
          i < 8 ? 'online' : (statuses[Math.floor(Math.random() * statuses.length)] ?? 'online'),
        location: {
          x: 100 + (i % 4) * 150,
    ...
    const calculateStats = (nodes: NetworkNode[]): NetworkStats => {
      ...
      return {
        ...
        performance: {
          avgLatency:
            onlineNodes.reduce((sum, node) => sum + node.metrics.latency, 0) / onlineNodes.length ||
            0,
          ...
          <div className='space-y-2 text-sm'>
            ...
            <div className='flex items-center justify-between'>
              ...
              <span className='capitalize'>
                {selectedNode.type === 'quantum'
                  ? 'كمي'
                  : selectedNode.type === 'classical'
                    ? 'كلاسيكي'
                    : 'مختلط'}
              </span>
            </div>
          ...
          {[
            {
              time: '14:32',
              type: 'blocked',
              message: 'تم حجب محاولة اختراق من IP: 192.168.1.100',
            },
