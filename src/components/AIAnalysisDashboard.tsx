...
const AIAnalysisDashboard: React.FC<AIAnalysisDashboardProps> = ({
  ...
  const addMessage = (role: 'user' | 'assistant', content: string, isAnalysis = false): string => {
    ...
    setMessages(prev => [
      ...prev,
      { id, role, content, timestamp: new Date(), isAnalysis },
    ]);
    return id;
  };
        ...
        <div className="text-center bg-gray-800/50 backdrop-blur-sm rounded-xl p-8 border border-yellow-500/30 max-w-md">
          ...
          <div className="bg-gray-900/80 rounded-lg p-4 text-left direction-ltr">
            <code className="text-green-400 text-sm">
              VITE_OPENAI_API_KEY=gsk_your_key_here
            </code>
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="w-12 h-12 bg-blue-500/20 rounded-lg flex items-center justify-center">
            <BarChart3 className={`w-6 h-6 text-blue-400 ${activeAnalysis === 'system' ? 'animate-pulse' : ''}`} />
          </div>
        ...
        >
          <div className="w-12 h-12 bg-green-500/20 rounded-lg flex items-center justify-center">
            <Shield className={`w-6 h-6 text-green-400 ${activeAnalysis === 'security' ? 'animate-pulse' : ''}`} />
          </div>
          <div className="text-right">
        ...
        >
          <div className="w-12 h-12 bg-purple-500/20 rounded-lg flex items-center justify-center">
            <Atom className={`w-6 h-6 text-purple-400 ${activeAnalysis === 'quantum' ? 'animate-pulse' : ''}`} />
          </div>
          <div className="text-right">
      ...
      {/* منطقة المحادثة */}
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 flex flex-col" style={{ height: '500px' }}>
        {/* رأس المحادثة */}
