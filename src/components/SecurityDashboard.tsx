    ...
    const generateThreats = (): SecurityThreat[] => {
      ...
      return Array.from({ length: 15 }, (_, i) => {
        ...
        return {
          ...
          affectedSystems: [
            `نظام ${Math.floor(Math.random() * 5) + 1}`,
            `خادم ${Math.floor(Math.random() * 3) + 1}`,
          ],
      ...
      {threats.map(threat => (
        ...
        >
          <div className='flex items-center justify-between mb-4'>
            ...
            <div className='flex items-center space-x-2 space-x-reverse'>
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${getSeverityColor(threat.severity)}`}
              >
                {getSeverityText(threat.severity)}
              </span>
              <span
      ...
      {accessLogs.slice(0, 20).map(log => (
        <div key={log.id} className='bg-white dark:bg-gray-800 rounded-lg p-4 shadow'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center space-x-3 space-x-reverse'>
              <div
                className={`w-3 h-3 rounded-full ${log.success ? 'bg-green-500' : 'bg-red-500'}`}
              />
              <div>
      ...
      {[
        {
          ...
          label: 'جدار الحماية',
          icon: Shield,
          description: 'حماية الشبكة من التهديدات الخارجية',
        },
        {
          key: 'antivirus',
          label: 'مكافح الفيروسات',
          icon: ShieldCheck,
          description: 'فحص وإزالة البرمجيات الخبيثة',
        },
        { key: 'encryption', label: 'التشفير', icon: Lock, description: 'تشفير البيانات الحساسة' },
        {
          ...
          icon: HardDrive,
          description: 'حفظ نسخ احتياطية من البيانات',
        },
        {
          key: 'monitoring',
          label: 'المراقبة',
          icon: Activity,
          description: 'مراقبة النشاط والتهديدات',
        },
                ...
                <div>
                  ...
                  <ul className='space-y-1'>
                    {selectedThreat.mitigationSteps.map((step, index) => (
                      <li
                        key={index}
                        className='flex items-center space-x-2 space-x-reverse text-gray-600 dark:text-gray-400'
                      >
