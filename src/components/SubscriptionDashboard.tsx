  ...
  return (
    <div className='bg-gray-800/50 rounded-xl p-4 border border-gray-700/50'>
      <div className='flex items-center justify-between mb-3'>
        ...
        <span
          className={`text-xs font-mono ${isCritical ? 'text-red-400' : isWarning ? 'text-yellow-400' : 'text-gray-400'}`}
        >
          {isUnlimited
            ? `${used.toLocaleString()} / ∞`
      ...
      <div className='fixed inset-0 z-[90] bg-black/70 backdrop-blur-md flex items-center justify-center p-4'>
        <div className='bg-gray-800 rounded-2xl p-8 max-w-md w-full border border-gray-700 text-center'>
          ...
          <p className='text-gray-400 mb-6'>
            يرجى تسجيل الدخول أو إنشاء حساب للوصول لإدارة الاشتراك.
          </p>
          ...
          <div className='bg-gray-800/50 rounded-xl p-6 border border-gray-700/50'>
            <div className='flex items-start justify-between mb-6'>
              <div>
                <div className='flex items-center gap-2 mb-1'>
                  ...
                  <h3 className='text-xl font-bold text-white'>خطة {currentPlan.name}</h3>
                  <span
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 gap-3 mb-6'>
              {[
                {
                  label: 'الكيوبتات',
                  value: limits.maxQubits === 128 ? '128+' : `${limits.maxQubits}`,
                  icon: <Atom className='w-4 h-4' />,
                },
                {
                  label: 'الخوارزميات',
                  value: !isFinite(limits.maxAlgorithms) ? '∞' : `${limits.maxAlgorithms}`,
                  icon: <Zap className='w-4 h-4' />,
                },
                {
                  label: 'أمان ما بعد الكم',
                  value: limits.hasPostQuantumSecurity ? 'مفعل' : 'غير متوفر',
                  icon: <Shield className='w-4 h-4' />,
                },
                {
                  label: 'SLA',
                  value: `${limits.slaUptime}%`,
          ...
          <div>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 gap-3'>
              {[
                ...
                {
                  label: 'دوائر مخصصة',
                  enabled: limits.hasCustomCircuits,
                  icon: <Atom className='w-4 h-4' />,
                },
                {
                  label: 'سجلات التدقيق',
                  enabled: limits.hasAuditLogs,
                  icon: <Shield className='w-4 h-4' />,
                },
                {
                  label: 'SSO / SAML',
                  enabled: limits.hasSso,
                  icon: <Settings className='w-4 h-4' />,
                },
                {
                  label: 'دعم مخصص',
                  enabled: limits.hasDedicatedSupport,
                  icon: <Star className='w-4 h-4' />,
                },
                {
                  label: 'White Label',
                  enabled: limits.hasWhiteLabel,
                  icon: <Crown className='w-4 h-4' />,
                },
                {
                  label: 'نماذج AI مخصصة',
                  enabled: limits.hasCustomModels,
                  icon: <Brain className='w-4 h-4' />,
                },
                {
                  label: 'أمان ما بعد الكم',
                  enabled: limits.hasPostQuantumSecurity,
                  icon: <Shield className='w-4 h-4' />,
                },
              ].map((f, i) => (
                <div
                ...
                >
                  ...
                  <span className={`text-sm ${f.enabled ? 'text-gray-200' : 'text-gray-500'}`}>
                    {f.label}
                  </span>
            ...
            <div className='grid grid-cols-2 gap-4 text-sm'>
              ...
              <div>
                ...
                <p className='text-white'>
                  {new Date(user.subscription.startDate).toLocaleDateString('ar-SA')}
                </p>
              </div>
            </div>
          </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <ul className='space-y-2 mb-4 text-xs text-gray-300'>
                          ...
                          <li className='flex items-center gap-1.5'>
                            <CheckCircle className='w-3.5 h-3.5 text-green-400' />
                            {!isFinite(plan.limits.maxSimulationsPerMonth)
                              ? '∞'
                              : plan.limits.maxSimulationsPerMonth.toLocaleString()}{' '}
                            محاكاة/شهر
                          </li>
                          <li className='flex items-center gap-1.5'>
                            <CheckCircle className='w-3.5 h-3.5 text-green-400' />
