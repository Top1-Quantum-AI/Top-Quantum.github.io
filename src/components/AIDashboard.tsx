...
const AIDashboard: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'overview' | 'models' | 'tasks' | 'analytics'>(
    'overview'
  );
  ...
  useEffect(() => {
    const generateModels = (): AIModel[] => {
      ...
      const statuses: Array<'training' | 'ready' | 'deployed' | 'error'> = [
        'training',
        'ready',
        'deployed',
        'error',
    ...
    const calculateStats = (models: AIModel[], tasks: AITask[]): AIStats => {
      ...
      return {
        ...
        averageAccuracy:
          activeModels.reduce((sum, model) => sum + model.accuracy, 0) / activeModels.length || 0,
        resourceUsage: {
    ...
    const interval = setInterval(() => {
      ...
      setStats(prev => ({
        ...
        resourceUsage: {
          ...
          memory: Math.max(
            10,
            Math.min(100, prev.resourceUsage.memory + (Math.random() - 0.5) * 3)
          ),
          gpu: Math.max(5, Math.min(100, prev.resourceUsage.gpu + (Math.random() - 0.5) * 8)),
          storage: Math.max(
            20,
            Math.min(100, prev.resourceUsage.storage + (Math.random() - 0.5) * 2)
          ),
        },
      }));
    }, 3000);
      ...
      {models.map(model => (
        ...
        >
          <div className='flex items-center justify-between mb-4'>
            ...
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(model.status)}`}
            >
              {getStatusText(model.status)}
            </span>
          </div>
      ...
      {tasks.map(task => (
        ...
        >
          <div className='flex items-center justify-between mb-4'>
            ...
            <div className='flex items-center space-x-3 space-x-reverse'>
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(task.priority)} bg-opacity-10`}
              >
                {task.priority === 'critical'
                  ? 'حرج'
                  : task.priority === 'high'
          ...
          {task.status === 'running' && (
            <div className='mb-4'>
              ...
              <div className='flex items-center justify-between text-xs text-gray-500 mt-1'>
                ...
                <span>
                  معدل الإنجاز: {(((task.progress / 100) * 60) / task.estimatedTime).toFixed(1)}
                  %/دقيقة
                </span>
              </div>
            </div>
          ...
          {viewMode === 'analytics' && (
            <div className='text-center py-12'>
              ...
              <p className='text-gray-600 dark:text-gray-400'>
                قريباً - تحليلات متقدمة لأداء النماذج
              </p>
            </div>
          )}
        </div>
                  ...
                  <div>
                    ...
                    <div className='space-y-2 text-sm'>
                      ...
                      <div className='flex justify-between'>
                        ...
                        <span
                          className={`px-2 py-1 rounded text-xs ${getStatusColor(selectedModel.status)}`}
                        >
                  ...
                  <div>
                    ...
                    <div className='space-y-3'>
                      {[
                        {
                          label: 'السرعة',
                          value: selectedModel.performance.speed,
                          unit: 'ops/s',
                          color: 'blue',
                        },
                        {
                          label: 'الذاكرة',
                          value: selectedModel.performance.memory,
                          unit: '%',
                          color: 'green',
                        },
                        {
                          label: 'المعالج',
                          value: selectedModel.performance.cpu,
                          unit: '%',
                          color: 'yellow',
                        },
                        {
                          label: 'كرت الرسوميات',
                          value: selectedModel.performance.gpu,
                          unit: '%',
                          color: 'purple',
                        },
