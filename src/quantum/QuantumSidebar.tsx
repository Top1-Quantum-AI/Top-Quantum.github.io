...
import type { QuantumModule } from './types';

type ModuleResearchData = { results?: unknown[] };

interface QuantumSidebarProps {
  quantumModules: QuantumModule[];
  activeModule: string;
  researchData: Record<string, ModuleResearchData>;
  onModuleSelect: (moduleId: string, researchTopic?: string) => void;
}

/** القائمة الجانبية لوحدات النظام الكمي */
      ...
      <div className="p-6">
        ...
        <div className="space-y-2">
          {quantumModules.map((module) => {
            ...
            const moduleData = researchData[module.id] as ModuleResearchData | undefined;
            return (
              <button
                key={module.id}
                onClick={() => onModuleSelect(module.id, module.researchTopic)}
              ...
              >
                ...
                {moduleData && (
                  <div className="mt-2 text-xs text-green-400">
                    ✓ تم البحث: {moduleData.results?.length ?? 0} نتائج
                  </div>
                )}
              </button>
            );
